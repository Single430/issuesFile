# coding=utf-8
# Implements API for MiniCPM-Llama3-V-2_5 in OpenAI's format. (https://platform.openai.com/docs/api-reference/chat)
# Usage: python minicpm_openai_api.py.py
# http://localhost:9433
...
class ChatMessage(BaseModel):
    ...
    image: Optional[str] = None  # base64.b64decode(base64_data)
...
def parse_messages(messages, functions):
    ...
    for m_idx, m in enumerate(_messages):
        role, content, func_call, image_str = m.role, m.content, m.function_call, m.image
        if content:
        ...
        if role == "function":
            ...
            t_content = content.lstrip("\n").rstrip()
            messages.append(
                ChatMessage(role="user", content=t_content, image=image_str)
            )
        else:
            raise HTTPException(
    ...
    for i in range(0, len(messages), 2):
        if messages[i].role == "user" and messages[i + 1].role == "assistant":
            ...
            history.append({"role": messages[i].role, "content": messages[i].content})
            history.append({"role": messages[i+1].role, "content": messages[i+1].content})
        else:
            raise HTTPException(
                status_code=400,
    ...
    if system:
        ...
        query = f"{system}\n\nQuestion: {query}"
    return query, history, image_data
...
async def create_chat_completion(request: ChatCompletionRequest):
    ...
    query, history, image_data = parse_messages(request.messages, request.functions)
    print("Old:", query, history)
    if request.stream:
        if request.functions:
            raise HTTPException(
                status_code=400,
    ...
    msgs = json.dumps(history, ensure_ascii=True)
...
def _get_args():
    DEFAULT_CKPT_PATH = '/home/zbl/seres/llms/MiniCPM-Llama3-V-2_5'
    ...
    parser.add_argument(
        "--server-port", type=int, default=9433, help="Demo server port."
    )
    parser.add_argument(
        "--server-name",
        type=str,
...
if __name__ == "__main__":
    ...
    if 'int4' in args.checkpoint_path:
        if device_map == 'mps':
            print('Error: running int4 model with bitsandbytes on Mac is not supported right now.')
            exit()
        model = AutoModel.from_pretrained(args.checkpoint_path, trust_remote_code=True)
    else:
        model = AutoModel.from_pretrained(args.checkpoint_path, trust_remote_code=True).to(dtype=torch.float16)
        model = model.to(device=device_map)
